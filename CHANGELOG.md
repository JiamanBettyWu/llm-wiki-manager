...
### Added
- **Audit mode** (`references/audit-workflow.md`, `scripts/audit_measure.py`): the process for turning a source page that has outgrown its source — a lecture page carrying every instructor slip, prediction and tally alongside the teaching — back into a *map*: measure against healthy siblings, triage errata by whether a reader who believed them would get the material wrong, move the audit trail to a per-module threads note, grep and repoint inbound heading anchors, rewrite the page as a bulleted map with concept-page pointers and review questions, trim the concept page, bookkeep. Distilled from three conversions (10.9k → 1k, 22.9k → 1.4k, 12.6k → 2k words). The wiki's own `AGENTS.md` owns the budgets and page shape; this mode owns the process.
- Lint check **"Pages over their length budget"** (quality tier): a source page whose frontmatter lists several `lectures:` is measured in body words per lecture (`--max-words-per-lecture`, default 400); a page under `concepts/` in body words (`--concept-max-words`, default 2000). A lecture page is a *map* of its lectures and a concept page is what a reader reviews from; past the budget a page has stopped compressing its source and the reader goes back to the video. Motivated by three lecture pages that had grown to 2–4× their lectures' watch time without any existing check firing. Fenced code blocks are not counted.
...
- Timestamp citations may carry a **module label**, `M4 L3 @ 05:28`. Before, the `M4` stopped the citation pattern from matching at all, so a labelled citation was skipped silently — the natural way to write a concept page that quotes two modules of one course side by side was the one form the check could not see. The label narrows the candidate transcripts to pages whose `module:` frontmatter matches: the citing page, then the pages it links, then any page in the wiki. An explicit `[[slug]]` still wins, and the quote's words still pick among the candidates.
- Lint check **"Broken heading anchors"** (quality tier; formerly "Broken in-page anchors"): now checks cross-page heading links `[[page#Section]]` as well as same-page `[[#Section]]`. The dangling-link check reads only a link's page part, so a renamed heading on another page left every link into it looking correct while navigating nowhere — the failure mode of hub pages built from links into other pages' sections. Block references (`#^id`), attachment links and links to missing pages are skipped; a nested `a#b#c` link compares its last segment.
